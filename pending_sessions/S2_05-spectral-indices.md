---
title: "Cálculo de índices espectrales"
teaching: 60
exercises: 60
questions:
- ¿Cómo obtener la mediana de una imagen en un periodo determinado?
- ¿Cómo calcular el índice de vegetación NDVI?
objectives:
- Acceder a imágenes Sentinel-2
- Obtener una imagen agregada 
- Calcular una banda adicional con el índice NDVI 
keypoints:
- Nuevas bandas representando índices espectrales pueden generarse fácilmente
- Es posible calcular cualquier índice aplicando *algebra de imágenes*
---
...
# Ejercicio: Cálculo del índice de vegetación
...
var colombia = ee.FeatureCollection("FAO/GAUL/2015/level1")
                  .filter(ee.Filter.eq('ADM0_NAME', 'Colombia'));//realice un filtrado para encontrar nuestro país 
                  
var valle = colombia.filter(ee.Filter.eq('ADM1_NAME', "Valle Del Cauca")); //otro filtro para obtener los límites del departamento  Valle

Map.centerObject(valle,9); // centrar el mapa en la zona de interes
Map.addLayer(valle, {color: 'FFFFFF', fillColor: '00000000'}, 'valle'); // ver el Valle transparente
...
print(s2a, 'Image Collection Valle 2020');
Map.centerObject(s2a,9)


//Step 3: Create a single Image by reducing by Min and clip it to the extent of the geometry
var s2a_min = s2a.min()
                    .clip(valle);

//Print your Image to your console tab to inspect it
print(s2a_min, 'Min reduced Image Valle 2020');

//Add your Image as a map layer
var visParams = {'min': 400,'max': [4000,3000,3000],   'bands':'B8,B4,B3'};
Map.addLayer(s2a_min, visParams, 'S2 Valle 2020 Min');
{% endhighlight %}

Departamento de interés: Valle
<br>
<img src="../fig/05_ValleRGB843.png" border = "10">
<br><br>
